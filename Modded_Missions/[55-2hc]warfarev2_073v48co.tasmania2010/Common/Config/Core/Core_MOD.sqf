...
_i = _i + [['MiG-21','',49999,50,5,-2,3,3,0,'Molatian Air Force',[]]];
    ...
    for '_z' from 0 to (count _c)-1 {
        if (isClass (configFile >> 'CfgVehicles' >> (_c select _z))) {
            missionNamespace getVariable (_c select _z);
            if (isNil '_get') {
                if ((_i select _z) select 0 == '') {(_i select _z) set [0, [_c select _z,'displayName'] Call GetConfigInfo]};
                if (typeName ((_i select _z) select 4) == 'SCALAR') {
                    if (((_i select _z) select 4) == -2) {
                        _ret = (_c select _z) Call Compile preprocessFile "Common\Functions\Common_GetConfigVehicleCrewSlot.sqf";
                        (_i select _z) set [4, _ret select 0];
                        (_i select _z) set [9, _ret select 1];
                    };
                };
                if (WF_Debug) {(_i select _z) set [3,1]};
                _p = if ((_c select _z) isKindOf 'Man') {'portrait'} else {'picture'};
