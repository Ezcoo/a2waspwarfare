...
_templates_label = [];
for '_i' from 0 to count(_templates)-1 do {[_templates_label, (_templates select _i) select 1] Call WFBE_CO_FNC_ArrayPush};

_templates_sorted = [];
_sorted = ([_templates_label] Call CIPHER_SortArrayIndex) select 1;

for '_i' from 0 to count(_templates)-1 do {[_templates_sorted, _templates select (_sorted select _i)] Call WFBE_CO_FNC_ArrayPush};
missionNamespace setVariable [Format ["WFBE_%1_Template", WFBE_Client_SideJoinedText], _templates_sorted];