...
missionNamespace setVariable [Format['WFBE_%1_ARTILLERY_DISPLAY_NAME', _side], ['M119','M252','MLRS','Stryker MC', 'MLRS (miner)']]; //--- Display Name to use in the GUI.
missionNamespace setVariable [Format['WFBE_%1_ARTILLERY_WEAPONS', _side], ['M119','M252','MLRS','M120', 'MLRS']]; //--- Weapon classname.
missionNamespace setVariable [Format['WFBE_%1_ARTILLERY_RANGES_MIN', _side], [1000,50,1200,550, 200]]; //--- Unit cannot fire if the target is within it's min range.
missionNamespace setVariable [Format['WFBE_%1_ARTILLERY_RANGES_MAX', _side], [7000,5500,9000,6500, 2000]]; //--- Unit max firing range.
...
missionNamespace setVariable [Format['WFBE_%1_ARTILLERY_BURST', _side], [8,4,6,6, 12]]; //--- Burst sent per fire mission.
missionNamespace setVariable [Format['WFBE_%1_ARTILLERY_VELOCITIES', _side], [500,475,550,475, 300]]; //--- Projectile fall velocity.
missionNamespace setVariable [Format['WFBE_%1_ARTILLERY_DISPERSIONS', _side], [50,60,40,55, 100]]; //--- Accuracy of the shell upon landing.
...
missionNamespace setVariable [Format['WFBE_%1_ARTILLERY_AMMO_MINES', _side], ['R_GRAD']]; //--- Mine rounds
