		...
		if (_currentRow != -1) then {
			...
			if !(_isInfantry) then {
				if (isClass (configFile >> 'CfgVehicles' >> _unit >> 'Library')) then {
					...
					if ([[[([[missionNamespace getVariable Format ['WFBE_%1_ARTILLERY_CLASSNAMES', sideJoinedText]], _x] call BIS_fnc_arrayFindDeep), -1] call BIS_fnc_areEqual], false] call BIS_fnc_areEqual) then {
