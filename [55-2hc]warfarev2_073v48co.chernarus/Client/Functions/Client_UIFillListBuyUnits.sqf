...
lnbClear _listBox;
{
	...
	if (((_c select QUERYUNITUPGRADE) <= (_currentUpgrades select _value) && _addin) || (_addit&&_addin)) then {
		...
		if (([[missionNamespace getVariable Format ['WFBE_%1_ARTILLERY_CLASSNAMES', sideJoinedText]], str _x] call WFBE_CO_FNC_findVariableInNestedArray) != -1) then {lnbSetColor [_listBox,[_i,1],[1.0, 0.25, 0.25, 1.0]]};
		diag_log(_x);
		diag_log("WFBE_CO_FNC_findVariableInNestedArray result: " + str (([[missionNamespace getVariable Format ['WFBE_%1_ARTILLERY_CLASSNAMES', sideJoinedText]], _x] call WFBE_CO_FNC_findVariableInNestedArray) != -1));
		diag_log("WFBE_CO_FNC_findVariableInNestedArray (str _x):" + str (([[missionNamespace getVariable Format ['WFBE_%1_ARTILLERY_CLASSNAMES', sideJoinedText]], str _x] call WFBE_CO_FNC_findVariableInNestedArray) != -1));
