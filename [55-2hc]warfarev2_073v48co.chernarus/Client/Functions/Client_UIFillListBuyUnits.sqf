...
lnbClear _listBox;
{
	...
	if (((_c select QUERYUNITUPGRADE) <= (_currentUpgrades select _value) && _addin) || (_addit&&_addin)) then {
		...
		if ([[[([[missionNamespace getVariable Format ['WFBE_%1_ARTILLERY_CLASSNAMES', sideJoinedText]], _x] call BIS_fnc_arrayFindDeep), -1] call BIS_fnc_areEqual], false] call BIS_fnc_areEqual) then {lnbSetColor [_listBox,[_i,1],[1.0, 0.25, 0.25, 1.0]]};
		/*
		diag_log(_x);
