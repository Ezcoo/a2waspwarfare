...
lnbClear _listBox;
{
	...
	if (((_c select QUERYUNITUPGRADE) <= (_currentUpgrades select _value) && _addin) || (_addit&&_addin)) then {
		...
		if (_c in (missionNamespace getVariable Format ["WFBE_%1AMBULANCES", sideJoinedText])) then {diag_log "true"; lnbSetColor [_listBox,[_i,0],[1.0, 1.0, 0.0, 1.0]]} else {diag_log "false"};
