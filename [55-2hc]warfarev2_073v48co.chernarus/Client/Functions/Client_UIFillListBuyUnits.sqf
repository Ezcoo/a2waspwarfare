...
lnbClear _listBox;
{
	...
	if (((_c select QUERYUNITUPGRADE) <= (_currentUpgrades select _value) && _addin) || (_addit&&_addin)) then {
		...
		if (_c in (missionNamespace getVariable Format ["WFBE_%1AMBULANCES", sideJoinedText])) then {diag_log "true"; lnbSetColor [_listBox,[_i,0],[1.0, 1.0, 0.0, 1.0]]} else {diag_log Format [missionNamespace getVariable Format ["WFBE_%1AMBULANCES", sideJoinedText]]; diag_log (missionNamespace getVariable Format ["WFBE_%1AMBULANCES", sideJoinedText])};
		if (_c in (missionNamespace getVariable Format ['WFBE_%1REPAIRTRUCKS',sideJoinedText])) then {diag_log "true"; lnbSetColor [_listBox,[_i,0],[1.0, 0.5, 0.25, 1.0]]} else { diag_log "false"};
		if (_c in (missionNamespace getVariable Format["WFBE_%1SUPPLYTRUCKS", sideJoinedText])) then {diag_log "true"; lnbSetColor [_listBox,[_i,0],[0.25, 0.75, 0.25, 1.0]]} else {diag_log "false";};
		_i = _i + 1;
	};
	_u = _u + 1;
} forEach _listNames;
